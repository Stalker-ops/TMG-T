...
class Buttons(sweetness.ui.View):
    ...
    @sweetness.ui.button(label="Выдача мута", style=sweetness.ButtonStyle.grey)
    async def mute(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Тип мута", icon_url='attachments')
        embed.description = "> Для того, чтобы выдать мут, вам нужно выбрать тип мута указаные ниже."
        embed.set_footer(text=f"Взаимодействие с пользователем - {self.member}", icon_url=self.member.display_avatar.url)
        embed.set_thumbnail(url=interaction.author.display_avatar)
        embed.color = 0x2f3136
        await self.message.edit(embed=embed, view=Mute(client=self.client, member=self.member, message=self.message))

    @sweetness.ui.button(label="Снятие мута", style=sweetness.ButtonStyle.grey)
    async def unmute(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Снятие мута", icon_url = "attachments")
        embed.description = "ᅠ\n> Выберите тип мута, который хотите снять :"
        embed.set_footer(text=f"Взаимодействие с пользователем - {self.member}", icon_url=self.member.display_avatar.url)
        embed.set_thumbnail(url=interaction.author.display_avatar)
        embed.color = 0x2f3136
        await self.message.edit(embed=embed, view=UnMute(client=self.client, member=self.member, message=self.message))

    @sweetness.ui.button(label="Выдача предупреждения", style=sweetness.ButtonStyle.grey)
    async def warn(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Тип предупреждения", icon_url='attachments')
        embed.description = "> Для того, чтобы выдать предупреждение, вам нужно выбрать тип предупреждения указаные ниже."
        embed.set_footer(text=f"Взаимодействие с пользователем - {self.member}", icon_url=self.member.display_avatar.url)
        embed.set_thumbnail(url=interaction.author.display_avatar)
        embed.color = 0x2f3136
    ...
    async def warnd(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Снятие предупреждения", icon_url = "attachments")
    ...
    @sweetness.ui.button(label="Выдача бана", style=sweetness.ButtonStyle.grey)
    async def ban(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Локальный бан", icon_url='attachments')
        embed.description = "> Для того, чтобы выдать локальный бан, нажмите на кнопку указанную ниже."
        embed.set_footer(text=f"Взаимодействие с пользователем - {self.member}", icon_url=self.member.display_avatar.url)
        embed.set_thumbnail(url=interaction.author.display_avatar)
        embed.color = 0x2f3136
        await self.message.edit(embed=embed, view=Ban(client=self.client, member=self.member, message=self.message))

    @sweetness.ui.button(label="Снятие бана", style=sweetness.ButtonStyle.grey)
    async def unban(self, button: sweetness.ui.Button, interaction: sweetness.MessageInteraction):
        embed = sweetness.Embed()
        embed.set_author(name="Снятие локального бана", icon_url = "attachments")
        embed.description = "ᅠ\n> Нажмите на кнопку ниже если хотите снять локальный бан:"
        embed.set_footer(text=f"Взаимодействие с пользователем - {self.member}", icon_url=self.member.display_avatar.url)
        embed.set_thumbnail(url=interaction.author.display_avatar)
        embed.color = 0x2f3136
        await self.message.edit(embed=embed, view=Unban(client=self.client, member=self.member, message=self.message))
